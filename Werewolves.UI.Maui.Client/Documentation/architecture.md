...
## 16. Test Infrastructure Architecture

### 16.1 Philosophy
The test infrastructure follows the **State Injection** pattern, allowing the QA Agent to "teleport" the UI to any game state instantly without simulating full game flows. This is based on the principle that the UI is a pure function of state: `f(State) = View`.
...
├── Mocks/
│   ├── MockGameSession.cs      # IGameSession implementation with public setters
│   ├── MockGameService.cs      # IGameService implementation with state injection
│   ├── MockPlayer.cs           # IPlayer implementation
│   └── MockPlayerState.cs      # IPlayerState implementation
├── TestInstructions/
│   └── README.md               # (Instruction subclasses planned)
├── StateFixtures/
│   └── README.md               # (JSON fixtures planned)
├── Infrastructure/
│   ├── TestStateDto.cs         # DTO for state injection
│   └── TestBridgeService.cs    # JS bridge C# endpoint
└── Tests/
    └── README.md               # (Test classes planned)
```
...
### 16.4 Dependency Injection for Testing

Production and test environments use different `IGameService` implementations:

**Production (`MauiProgram.cs`):**
```csharp
...
### 16.5 IGameService Interface

A local `IGameService` interface (`Services/IGameService.cs`) abstracts the Core's `GameService`:
...
**Note:** This interface is a workaround until Core exposes its own `IGameService`. The `GameServiceWrapper` adapts Core's concrete `GameService` to this interface.

### 16.6 Mock Implementation Pattern

Mock classes implement the same interfaces as production classes but:
- Have public setters for all properties (no readonly)
- Contain no game logic (pure state containers)
...
public class MockGameSession : IGameSession
{
    ...
    public void AddPlayer(MockPlayer p) { ... }  // Test helper
}
```

### 16.7 JavaScript Bridge

The `window.TestBridge` object (defined in `wwwroot/index.html`) enables the QA Agent to inject state at runtime via Chrome DevTools Protocol:
...
window.TestBridge = {
    ...
    setState: function(jsonString) {
        return this._dotNetRef.invokeMethodAsync('InjectState', jsonString);
    },
    isReady: function() { return !!this._dotNetRef; }
};
```

**State Injection Flow:**
1. QA Agent calls `window.TestBridge.setState(json)` via Chrome DevTools
2. JS → C# interop via `[JSInvokable]` on `TestBridgeService`
3. `TestBridgeService.InjectState()` parses JSON into `TestStateDto`
4. `MockGameService.ApplyTestState()` updates mock session
5. `GameClientManager.NotifyStateChanged()` fires `StateChanged` event
6. Blazor components re-render with new state

### 16.8 Test Scope Definition
...
### 16.9 GameClientManager Test Helpers

The `GameClientManager` exposes internal methods for test infrastructure:

```csharp
// Force UI refresh
internal void NotifyStateChanged() => StateChanged?.Invoke();

// Set instruction without going through game flow
internal void SetCurrentInstruction(ModeratorInstruction instruction);

// Set session without starting a game
